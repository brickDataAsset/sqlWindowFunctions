...
NTILE(5) OVER(ORDER BY salary DESC) group_num
FROM employees;
...
NTILE(5) OVER(PARTITION BY department
			  ORDER BY salary DESC) group_num
FROM employees;
...
SELECT salary_rank, ROUND(AVG(salary),2) avg_salary
FROM salary_ranks
...
COUNT(*) OVER(PARTITION BY department
			 ORDER BY department ASC) dept_count
FROM employees;
...
SUM(salary) OVER(PARTITION BY department
				ORDER BY hire_date) running_total
FROM employees;
...
SUM(salary) OVER(ORDER BY hire_date
				RANGE BETWEEN
				UNBOUNDED PRECEDING AND CURRENT ROW) running_total
FROM employees;
...
SUM(salary) OVER(ORDER BY hire_date
				ROWS BETWEEN
				1 PRECEDING AND CURRENT ROW) running_total
FROM employees;
...
ROUND(AVG(salary) OVER(ORDER BY hire_date
				ROWS BETWEEN
				1 PRECEDING AND CURRENT ROW),2) running_avg
FROM employees;
...
SELECT ship_mode, category, sub_category, SUM(quantity) AS quantity
FROM sales
